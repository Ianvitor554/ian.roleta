# -*- coding: utf-8 -*-
"""
IR PREDICTOR 360 WEB LIVE V6 CLOUD 24H - LABORATÓRIO DE ESTRATÉGIAS
Pydroid 3 / Android
...
def collector_loop():
    print(f"[collector] iniciado; intervalo={COLLECT_INTERVAL:.1f}s")
    while True:
        try:
            added = engine.fetch(force=True)
            if added:
                print(f"[collector] +{added} rodada(s); total={len(engine.history)}")
        except Exception as exc:
            print("[collector] erro:", exc)
        time.sleep(COLLECT_INTERVAL)

def start_collector():
    global _collector_started
    if not RUN_COLLECTOR:
        print("[collector] desativado por RUN_COLLECTOR=0")
        return
...
<title>IR Predictor 360 V6 Cloud 24H</title>
<style>
:root{
...
function render(d){
 ...
 document.getElementById('cloudPanel').innerHTML=
   ...
   `<span class="small">O coletor roda no servidor e não depende desta página permanecer aberta.</span>`;
...
if __name__ == "__main__":
    ...
    print("=" * 68)
    print("IR PREDICTOR 360 • V6 CLOUD 24H • SQLITE + AUTO-LEARNING")

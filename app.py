...
APP_VERSION = "3.5-mega-roulette-professional-learning"
...
async function refresh(){const d=await (await fetch('/api/status')).json();if(!d.ok){document.getElementById('sig').textContent='SEM DADOS';return}const c=d.current||{},sig=c.signal||'RED',se=document.getElementById('sig');se.className='signal '+(sig.startsWith('GREEN')?sig:sig);se.textContent=(sig==='GREEN_ELITE'?'🟢⚡ ':sig==='GREEN_PLUS'?'🟢🔥 ':sig==='GREEN'?'🟢 ':sig==='YELLOW'?'🟡 ':'🔴 ')+sig;document.getElementById('score').innerHTML=`<b>Qualidade ${Number(c.quality||0).toFixed(1)}/100</b> • limite ${Number(c.threshold||0).toFixed(1)}`;document.getElementById('reason').textContent=c.reason||'';const net=d.network||{},cl=d.cloud||{};document.getElementById('net').innerHTML=`<span class="badge ${net.state==='ONLINE'?'online':'offline'}">● ${esc(net.state||'')}</span><span class="badge">${cl.mode?'☁️ CLOUD 24/7':'📱 LOCAL'}</span><span class="badge">${d.spins||0} giros</span><span class="badge">backfill ${cl.last_backfill_pages||0} pág.</span>`;

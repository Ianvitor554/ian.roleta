...
APP_VERSION = "3.4-mega-roulette-adaptive-counter-cloud"
...
class ModelPerformanceTracker:
    ...
    def __init__(self,db):
        ...
        for name in MODEL_PRIORS:
            self.stats.setdefault(name,{"n":0,"ema_log_loss":None,"ema_brier":None,"ema_top5":None})

    def save(self):
        self.db.set_state(self.KEY,{"stats":self.stats})
    ...
    def evaluate(self,nums_before,actual,ai):
        ...
        actual=int(actual)
        alpha=MODEL_WEIGHT_EMA_ALPHA
        with self.lock:
            for name,p in models.items():
                st=self.stats.setdefault(name,{"n":0,"ema_log_loss":None,"ema_brier":None,"ema_top5":None})
                ll=logloss(p,actual); br=brier(p,actual); h=float(actual in topk(p,5))
                st["n"]=int(st.get("n",0))+1
                for key,val in (("ema_log_loss",ll),("ema_brier",br),("ema_top5",h)):
                    old=st.get(key)
                    st[key]=val if old is None else (1-alpha)*float(old)+alpha*val
            if max(int(x.get("n",0)) for x in self.stats.values())%5==0:
                self.save()
    ...
    def weights(self):
        ...
        for name,prior in MODEL_PRIORS.items():
            ...
            n=int(st.get("n",0)); ll=st.get("ema_log_loss")
            conf=clamp(n/max(1,MODEL_WEIGHT_MIN_SAMPLES),0,1)
            if ll is None:
                perf=1.0
            else:
                perf=math.exp(clamp((UNIFORM_LOG_LOSS-float(ll))/MODEL_WEIGHT_TAU,-1.2,1.2))
            # Enquanto há pouca amostra, fica perto do prior.
            scores[name]=max(0.01,float(prior)*((1-conf)+conf*perf))
    ...
    def metrics(self):
        ...
        for name in MODEL_PRIORS:
            ...
            out[name]={
                "n":int(st.get("n",0)),
                "ema_log_loss":st.get("ema_log_loss"),
                "ema_brier":st.get("ema_brier"),
                "ema_top5_pct":100*float(st.get("ema_top5") or 0.0),
                "weight_pct":100*weights.get(name,0.0),
                "beats_uniform":bool(st.get("ema_log_loss") is not None and float(st.get("ema_log_loss"))<UNIFORM_LOG_LOSS),
            }
...
class Engine:
    ...
    def ensemble(self):
        ...
        weights=self.model_tracker.weights()
        ...
        round_meta={
            ...
            "model_tracker":self.model_tracker.metrics(),
            "temperature":temp,
        }
    ...
    def status(self):
        ...
        return {
            ...
            "adaptive_models":self.model_tracker.metrics(),
...
<title>Mega Roulette AI V3.4</title><style>
...
</style></head><body><div class="wrap"><h1>🎯 Mega Roulette AI V3.4</h1><div class="muted">IA adaptativa • análise por rodada • puxadores • contador 0–36 • histórico integral • cloud 24/7</div><br><div class="grid">
...
const ad=d.adaptive_models||{},mods=ad.models||{};document.getElementById('models').innerHTML=Object.entries(mods).map(([k,v])=>`<div class="model"><b>${esc(k)}</b><br><strong>${Number(v.weight_pct||0).toFixed(1)}%</strong><br><span class="muted">n ${v.n||0} • log loss ${v.ema_log_loss==null?'—':Number(v.ema_log_loss).toFixed(3)} • TOP5 EMA ${Number(v.ema_top5_pct||0).toFixed(1)}%</span></div>`).join('');const t=d.temperature||{};document.getElementById('temp').textContent=`Calibração de temperatura: T=${Number(t.temperature||1).toFixed(2)} • amostra ${t.n||0} • melhora log loss ${Number(t.improvement||0).toFixed(4)}`;
const cal=d.calibration||{},m=d.metrics||{},a=d.ai||{};document.getElementById('validation').innerHTML=box('IA TOP5',Number(a.top5_pct||0).toFixed(2)+'%','base 13,51%')+box('Greens',m.green_count||0,Number(m.green_hit_pct||0).toFixed(1)+'% acerto')+box('Calibração ECE',Number(cal.ece||0).toFixed(1)+' pp','gap '+Number(cal.gap_pp||0).toFixed(1)+' pp')+box('Log loss final',m.avg_log_loss==null?'—':Number(m.avg_log_loss).toFixed(3),'uniforme 3,611');document.getElementById('cloud').innerHTML=box('Modo',cl.mode?'24/7':'LOCAL',net.state||'—')+box('Último sucesso',net.age_seconds==null?'—':Math.round(net.age_seconds)+'s','idade dos dados')+box('Recuperação',cl.last_backfill_new||0,'novos no último backfill')+box('SQLite',d.integrity||'—',d.spins+' giros');
...
def api_model_weights():
    return jsonify({"ok":True,"adaptive":engine.model_tracker.metrics(),

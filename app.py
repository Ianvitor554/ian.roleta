...
APP_VERSION = "3.5.1-mega-roulette-bootstrap-fix"
...
class Engine:
    ...
    def ensure_prediction(self):
        ...
        sig=make_signal(probs,self.nums(),self.ai,matrix,metrics,calibration,round_meta.get("model_weights"))
        ...
        sig,professional=apply_professional_gate(sig,edge,tracker_metrics,ai_metrics,puller_metrics)

        round_analysis=self.build_round_analysis(
            probs,sig,puller_report,round_meta,
            blend,ai_skill,puller_blend,puller_skill
        )
        round_analysis["professional"]=professional
        ...
        self.db.insert_prediction({
            ...
            "round_analysis":round_analysis,"model_version":APP_VERSION
        })
...
<title>Mega Roulette AI V3.5 Professional</title><style>
...
</style></head><body><div class="wrap"><h1>🎯 Mega Roulette AI V3.5 Professional</h1><div class="muted">IA profissional adaptativa • memória rápida/lenta • análise por rodada • puxadores • cloud 24/7</div><br><div class="grid">
...
<section class="card s12"><h2>🧠 Mesa profissional da IA</h2>
<div id="proDecision" class="prodecision">CARREGANDO...</div><div id="proReason" class="muted"></div>
<div id="proMetrics" class="probox" style="margin-top:10px"></div><div id="learningTrend" class="muted" style="margin-top:9px"></div></section>
<section class="card s12"><h2>🔬 Rodada atual</h2><div id="round"></div></section>
<section class="card s12"><h2>🎯 3 centros + vizinhos</h2><div id="centers" class="centers"></div><br><div id="wheel" class="wheel-line"></div><div id="coverage" class="muted" style="margin-top:8px"></div></section>
...
const W=[0,32,15,19,4,21,2,25,17,34,6,27,13,36,11,30,8,23,10,5,24,16,33,1,20,14,31,9,22,18,29,7,28,12,35,3,26];const REDS=new Set([1,3,5,7,9,12,14,16,18,19,21,23,25,27,30,32,34,36]);let spinOffset=0;const spinLimit=100;const esc=x=>String(x??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));const box=(a,b,c='')=>`<div class="metric"><span class="muted">${a}</span><br><b>${b}</b><br><span class="muted">${c}</span></div>`;
